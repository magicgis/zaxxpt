	...
	if (node.url) {
		...
		if (this.config.useSelection && ((node._hc && this.config.folderLinks) || !node._hc)) {
			...
			if (node.url.indexOf('javascript:') > -1)
			    ...
			    //javascript:addTabPanel("½ÇÉ«¹ÜÀí","/itsm/system/role_enter.action?menuid=3");
			
			    str += ' onclick= ' + node.url.replace('javascript:','');//substring(node.url.indexOf('javascript:'),node.url.length);
			else
