...
<title>机场休息室预订 - ${domain_cn }</title>
<script type="text/javascript">
<%--
$(document).ready(function(){
	var optMsg="${param.message}";
	var type="${param.type}";
if(optMsg=='succe'||optMsg=='error'){
  if(type=='3'){
	if(optMsg=='succe'){
	  alert("预定成功!")
	}else{
...
function jump(id)
	{
		window.location.href="${ctx}/loungeOrderAction!viewOrderDetail.action?id="+id;
	}
	function back()
	{
		window.location.href="${ctx}/lounge/index.jsp";
	}
	var isclick=false;
	function confirm(id)
	{
		if(isclick==false)
		{
			window.location.href="${ctx}/loungeOrderAction!verifyLoungeOrder.action?id="+id+"&&orderLoungeVo.id="+id+"&&type=N";
			isclick=true;
		}
	}
</script>
        ...
        <dl class="Tips">
            <s:if test='orderCode!=null&&orderCode!=""'>
           	<c:choose>
            	...
            	<c:if test="${param.type==3&&param.message!='error'&&param.message!='success'}">
            		<dd>尊敬的${_user_.memberRole.name}会员${_user_.name}，您已下单成功，${payMessage}请等待客服确认。如有疑问，请尽快致电您的专属秘书<span class="orange">${site_tel}</span>！</dd>
            		<a onclick="jump('${orderId}')" href="#">进入订单管理</a><br><br>
            	</c:if>
            	<c:if test="${param.type==3&&param.message=='success'}">
            		<dd>尊敬的${_user_.memberRole.name}会员${_user_.name}，您已预订成功，请注意预约日期和时间。如有疑问，请尽快致电您的专属秘书<span class="orange">${site_tel}</span>！</dd>
            		<a onclick="jump('${orderId}')" href="#">进入订单管理</a><br><br>
            	</c:if>
            	<c:if test="${param.type==3&&param.message=='error'}">
            		<dd>您的订单已经预定失败,请联系管理员,或者重新预定。</dd>
            	</c:if>
            	<c:if test="${param.type!=3&&param.message=='success'}">
            		...
            		<td width="50%">
            			<table border="0" width="100%">
            			<tr>
            			<td width="50%">订单号:<font color="red">${orderCode }</font></td>
            			<td> 订单总额:<font color="red">RMB ${price }</font></td>
            			</tr>
            			<tr>
            				<td>机场：${airportName }</td>
            			</tr>
            			<tr>
            				<td>休息室类型：<f:write type="休息室类型" value="${roomType}"/></td>
            			</tr>
            			<tr>
            				<td>日期：${bookTime}</td>
            			</tr>
            			<tr>
            				<td>旅客姓名：${passengerName}</td>
            			</tr>
            		    </table>
            		</td>
            		<td width="50%" valign="top">
            			...
            			<tr>
            				<td>登陆${domain_cn}网站订单管理人工退改或致电<span class="orange">${site_tel}</span>客服退改</td>
            			</tr>
            			</table>
            		</td>
            		</tr>
...
function showCalendar(dateType){
   ...
   case 2:
     ...
     break
        ...
        <c:if test="${param.type==3&&orderSts=='10'&&orderCode!=null&&orderCode!=''}">
        	<button type="button" class="Btn101_25" onclick="confirm('<s:property value="orderId"/>')">会员确认</button>
        </c:if>
        <s:if test='orderCode==null||orderCode==""'>
...
session.removeAttribute("bookstarttime");
