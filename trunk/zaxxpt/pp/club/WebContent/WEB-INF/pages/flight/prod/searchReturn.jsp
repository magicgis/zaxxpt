...
<head>
	...
	<script type="text/javascript">
		...
		var pageName="searchReturn.jsp"; var specialInfoName="respecialInfoJson";
		var returnFlag=true;
	</script>
...
<%@include file="/flight/controlReturn.jsp" %>
<div class="clear"></div>
<div class="search">
	<div class="search_left">
        <div class="searchform_frm">
                ...
                <div class="r_form w2">
                	<div class=inb>
                        <input id="plainDate1" class="inps" type="text" onfocus="WdatePicker({minDate:'%y-%M-%d'})"  name="searchDepartureTime" value="<%=departureDate%>" onclick="WdatePicker({maxDate:'#F{\'2050-10-01\'}',minDate:'<%=departureDate %>'})" onchange="checkWhenBack()"/>
                    	<!--     <a class="set_date" href="javascript:WdatePicker({el:'plainDate1',maxDate:'#F{\'2050-10-01\'}',minDate:'%y-%M-%d'})"></a>--%>
                    	-->
                    </div>
            	</div>
                <div class="left_txt w3 ReturnTime">
               	<span>返程时间：</span>
                </div>
...
<div id="btop" class="wfpj Wmargin">
        ...
        <div class="jp_hb_right w2">
        	<%if(!flightIsNullFlag){%>
        		出发日期：<%=departureDate%>（共<span class="red"><%=inFlightList.size()%></span>个航班信息）
        	<%}%>
        </div>
</div>
...
<!--list-->
<div class="main980">
		<div class="module_a Order marginB10">
	<%
		String reSelect = "search.jsp?flightType="+flightType+"&deAirport="+arrivalAirportCode+"&arAirport="+departureAirportCode+"&departureDate="+departureDate+"&returnDate="+returnDate;
	%>
        <div class="title"><span class="l"></span><span class="c"><b class="left">已选定的去程航班 </b><b class="left">&nbsp;<a class="blue" href=<%=reSelect%> >[重新选择去程]</a></b><b class="right">出发日期：<%=departureDate%></b></span><span class="r"></span></div>
				...
				<tbody>
					<tr>
                        <td class="airline">
                        	...
                        	<strong>
                        	<al:write key="<%=bookingGoFlightVo.getAirlineCompanyCode()%>"></al:write><br>
                            <%=bookingGoFlightVo.getAirlineCompanyCode()%><%=bookingGoFlightVo.getFlightNo()%>
                            </strong>
                        </td>
                        <td class="airtime">
                        	<%= bookingGoFlightVo.getDepartureTimeStr()%><br> 
                        ...
                        <td class="airprice">
                        	<%=bookingGoFlightVo.getCabinCode()%>舱<!--  <span class="red bold"> </span> 折 -->
                        </td>
                        <td class="airtax">
                        	<%=bookingGoFlightVo.getAdultConstructionFee()%>/<%=bookingGoFlightVo.getAdultBaf()%>
                        </td>
                        <td class="airtotal">
                        	<%=bookingGoFlightVo.getAdultTicketPrice().add(bookingGoFlightVo.getAdultConstructionFee()).add(bookingGoFlightVo.getAdultBaf()).toBigInteger()%><strong></strong>
                        <br />
                        <a href="#" class="orangea Tickets_TG">退改签规则</a>
                        <div class="Tickets_TGBox">
                        	<%=bookingGoFlightVo.getChangeInfo() %>
                        </div>
                        </td>
                    </tr>
				</tbody>
...
<!--	                        <th class="airorder"><a id="payPriceSorter" href="javascript:void(0);" class="air_up2">会员价<b></b></a></th>-->
        ...
        <%--					迭代所有的航班信息--%>
			<%for(int i=0;i<inFlightList.size();i++){
				FlightInfoVo flightInfoVo=inFlightList.get(i);
			%>
...
<%--        				虚拟存在的表头,用于保持页面对齐 --%>
	            	...
	            	<tbody>
						<tr class="Tickets">
							...
							<td class="airplane">
								<%=flightInfoVo.getConstructionFee()%>/
								<%=flightInfoVo.getAdultBaf()%>
							</td>
							<td class="airprice">
							...
							<td class="airorder" id="commonDataTd<%=i%>">
								...
								<input type="hidden" class="commonDataInput"  name="reflightInfoJson" value="<%=flightVoToJson(flightInfoVo)%>"/>
								<input type="hidden" class="uniqueDataInput"  name="recabinInfoJson" value="<%=cabinVoToJson(cabinInfoVo)%>"/>
							</td>
											...
											%>
												<tr id="">
													...
													<td class="airorder">
														...
														<input type="hidden" name="commonDataId" value="commonDataTd<%=i%>">
														<input type="hidden" class="uniqueDataInput"  name="recabinInfoJson" value="<%=cabinVoToJson(cabinInfoVoHide)%>"/>
													</td>
													<td class="vip" id="vip<%=flightInfoVo.getAirlineCompanyCode()+flightInfoVo.getFlightNo()+"_"+cabinInfoVoHide.getCabinCode()%>"></td>
													<td class="vip2"></td>
												</tr>
											<%}%>
										</tbody>
...
<%--	<input type="hidden" value="" id="bookingGoFlightVoJsonQ" name="bookingGoFlightVoJson">--%>
	<input type="hidden" class="commonDataInput"  name="flightInfoJson" id="flightInfoJson" value="${flightInfoJson }"/>
	<input type="hidden" class="uniqueDataInput"  name="cabinInfoJson" id="cabinInfoJson" value="${cabinInfoJson }"/>
	<input type="hidden" class="specialDataInput" name="specialInfoJson" id="specialInfoJson" value="${specialInfoJson}"/>
</form>
<form action="${ctx}/flight/book.jsp" method="post" id="flightBookForm">
	...
	<input type="hidden" name="returnDate" value="<%=returnDate%>" id="returnDate"/>
	<%--特价信息的json ,舱位信息的json ,特价信息的json  去程--%>
	<input type="hidden"  name="goflightInfoJson" id="flightInfoJson" value="${goflightInfoJson }"/>
	<input type="hidden"  name="gocabinInfoJson" id="cabinInfoJson" value="${gocabinInfoJson }"/>
	<input type="hidden"  name="gospecialInfoJson" id="specialInfoJson" value="${gospecialInfoJson}"/>
	<%--特价信息的json ,舱位信息的json ,特价信息的json 返程--%>
	<input type="hidden" class="commonDataInput"  name="reflightInfoJson" id="flightInfoJson" value=""/>
	<input type="hidden" class="uniqueDataInput"  name="recabinInfoJson" id="cabinInfoJson" value=""/>
	<input type="hidden" class="specialDataInput" name="respecialInfoJson" id="specialInfoJson" value=""/>
</form>
<script type="text/javascript">
